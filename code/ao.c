...
Scalar drand48(Ctx* ctx) {
    Scalar n = ((nrand(&ctx->rng)) / 65535.0f);
    n = n - floorf(n);
    return n;
}
...
void render_pixel(Ctx* ctx, int x, int y, int w, int h, int nsubsamples, unsigned int* img) {
    ...
    for (int i = 0; i < 10; i++) {
        pixel[0] = pixel[1] = pixel[2] += 0.1f * drand48(ctx);
    }*/

    img[0] = aobench_clamp(pixel[0]);
    img[1] = aobench_clamp(pixel[1]);
    img[2] = aobench_clamp(pixel[2]);
}
